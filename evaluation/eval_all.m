...
switch data_name
    case 'ADE20K'
        ...
        step = 2000 %500; %equals to number of images divide num of GPUs in testing e.g. 500=2000/4
        data_root = '/home/balloch/Data/ADE20K_2016_07_26'; %'/data2/hszhao/dataset/ADEChallengeData2016'; %root path of dataset
        eval_list = 'list/ADE20K_valwrobot.txt'; %evaluation list, refer to lists in folder 'samplelist'
        save_root = 'mc_result/Robotdata/pspnet50_1'%'mc_result/ADE20K/val/pspnet50_473/'; %root path to store the result image
...
acc = double.empty;
iou = double.empty;
gpu_id_array = 0 %[0:3]; %multi-GPUs for parfor testing, if number of GPUs is changed, remember to change the variable 'step'
runID = 1;
gpu_num = size(gpu_id_array,2);
index_array = [(runID-1)*gpu_num+1:runID*gpu_num];

for i = 1:gpu_num %parfor i = 1:gpu_num %change 'parfor' to 'for' if singe GPU testing is used
